...
func (m *model) availability(cfg config.FolderConfiguration, snap *db.Snapshot, file protocol.FileInfo, block protocol.BlockInfo) []Availability {
	...
	candidates = append(candidates, m.availabilityInTemporaryRLocked(cfg, file, block)...)
...
func (m *model) availabilityInTemporaryRLocked(cfg config.FolderConfiguration, file protocol.FileInfo, block protocol.BlockInfo) []Availability {
	var availabilities []Availability
